...
public:
    ...
    PatchWorkpp(patchwork_struct patchwork_va) {

        verbose_ = patchwork_va.verbose;
        sensor_height_ = patchwork_va.sensor_height;
        num_iter_ = patchwork_va.num_iter;
        num_lpr_ = patchwork_va.num_lpr;
        num_min_pts_ = patchwork_va.num_min_pts;
        th_seeds_ = patchwork_va.th_seeds;
        th_dist_ = patchwork_va.th_dist;
        th_seeds_v_ = patchwork_va.th_seeds_v;
        th_dist_v_ = patchwork_va.th_dist_v;
        max_range_ = patchwork_va.max_range;
        min_range_ = patchwork_va.min_range;
        uprightness_thr_ = patchwork_va.uprightness_thr;
        adaptive_seed_selection_margin_ = patchwork_va.adaptive_seed_selection_margin;
        RNR_ver_angle_thr_ = patchwork_va.RNR_ver_angle_thr;
        RNR_intensity_thr_ = patchwork_va.RNR_intensity_thr;
        max_flatness_storage_ = patchwork_va.max_flatness_storage;
        max_elevation_storage_ = patchwork_va.max_elevation_storage;
        enable_RNR_ = patchwork_va.enable_RNR;
        enable_RVPF_ = patchwork_va.enable_RVPF;
        enable_TGR_ = patchwork_va.enable_TGR;

        // ROS_INFO("1---Sensor Height: %f", sensor_height_);
        // ROS_INFO("2---Num of Iteration: %d", num_iter_);
        // ROS_INFO("3---Num of LPR: %d", num_lpr_);
        // ROS_INFO("4---Num of min. points: %d", num_min_pts_);
        // ROS_INFO("5---Seeds Threshold: %f", th_seeds_);
        // ROS_INFO("6---Distance Threshold: %f", th_dist_);
        // ROS_INFO("7---Max. range:: %f", max_range_);
        // ROS_INFO("8---Min. range:: %f", min_range_);
        // ROS_INFO("9---Normal vector threshold: %f", uprightness_thr_);
        // ROS_INFO("10---adaptive_seed_selection_margin: %f", adaptive_seed_selection_margin_);

        // CZM表示“同心区模型”。请参阅我们的论文
        num_zones_ = patchwork_va.num_zones;
        num_sectors_each_zone_ =  patchwork_va.num_sectors_each_zone;
        num_rings_each_zone_ =  patchwork_va.num_rings_each_zone;
        elevation_thr_ =  patchwork_va.elevation_thr;
        flatness_thr_  =  patchwork_va.flatness_thr;
        ...
        visualize_ = patchwork_va.visualize;
...
template<typename PointT> inline
void PatchWorkpp<PointT>::flush_patches(std::vector<Zone> &czm) {
    ...
    for (int zone_idx = 0; zone_idx < num_zones_; ++zone_idx) {
        ...
        for (int ring_idx = 0; ring_idx < num_rings_each_zone_[zone_idx]; ++ring_idx) {
            for (int sector_idx = 0; sector_idx < num_sectors_each_zone_[zone_idx]; ++sector_idx) {
                ...
                /*
                关于“is_heading_outside”条件，理论上heading应该小于0,可以尝试想象地平面上的曲面法向量与 连接传感器原点和地平面平均点的矢量
                但是当贴片远离传感器原点时， 航向可能大于0，即使它是地面，由于缺乏地平面点的数量。
                因此，我们只在concentre_idx<num_rings_of_enterest（接近条件）时检查此值
                */
                ...
                /*
                存储高程和平面度变量 用于A-GLE（自适应地面似然估计） 以及TGR（Temporal Ground Revert）。更多信息请参阅论文Patchwork++。
                */
